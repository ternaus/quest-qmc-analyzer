...
parser.add_argument('-min_beta', type=float, help="minim value of the inverse temperature")
...
for nSites in nSites_list:
  ...
  x, y, Err = common.merge.merge(splitted, args.y_variable)

  if args.x_variable == '1L':
    y = y[0] / nSites
    Err = Err[0] / nSites
...
errorbar(xList, yList, yerr=yErr, fmt='D-', linewidth=3,  markersize=15)

approxx, approxy, m, c, std_err = common.extrapolation.approx(xList, yList, yErr)

plot(approxx, approxy,'--', linewidth = 3, color='black')

text(0.05, 0.01, r'$\Delta^2 = {delta}$'.format(delta=c), fontsize=30)
